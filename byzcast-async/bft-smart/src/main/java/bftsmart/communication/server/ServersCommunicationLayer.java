...
public class ServersCommunicationLayer extends Thread {
    ...
    public ServersCommunicationLayer(ServerViewController controller,
		...
		try {
			fis = new FileInputStream("byzcast-async/config/keysSSL_TLS/" + controller.getStaticConf().getSSLTLSKeyStore());
//			fis = new FileInputStream(this.controller.getStaticConf().getConfigHome() + "/keysSSL_TLS/" + this.controller.getStaticConf().getSSLTLSKeyStore());
...
//            logger.error("Failed to serialize message", ex);
	        System.out.println("Failed to serialize message" + ex);
        }
...
//					System.out.println("Queueing (delivering) my own message, me:{}" + target);
				} else {
					logger.debug("Sending message from:{} -> to:{}.", me,  target);
//					System.out.println("Sending message from:{} -> to:{}." +  me + " - " +  target);
					getConnection(target).send(data);
				}
			} catch (InterruptedException ex) {
//				logger.error("Interruption while inserting message into inqueue", ex);
				System.out.println("Interruption while inserting message into inqueue" + ex);
			}
		}
    }
