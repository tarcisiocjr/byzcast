...
public class TOMMessage extends SystemMessage implements Externalizable, Comparable, Cloneable {
    ...
    public static byte[] messagesToBytes(TOMMessage[] reqs) {
        ...
        try {
            ...
            System.err.println("Unable to convert TOMMessage to bytes");
            e.printStackTrace();
            return null;
        }
    ...
    public static TOMMessage[] bytesToMessages(byte[] b) {
        ByteArrayInputStream in = new ByteArrayInputStream(b);
        TOMMessage[] reqs = null;
        int size;

        try {
            ObjectInputStream dis = new ObjectInputStream(in);
            size = dis.readInt();
            reqs = new TOMMessage[size];
            for (int i = 0; i < reqs.length; i++) reqs[i] = (TOMMessage) dis.readObject();


        } catch (IOException | ClassNotFoundException e) {
            System.err.println("Unable to convert bytes to TOMMessage");
            e.printStackTrace();
        }
        return reqs;
    }
