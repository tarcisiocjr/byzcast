...
public class CLIParser {
	private Option id, group, globalPercent, clientCount, duration, msgSize, nonGenuine, treeConfig, outstanding;
	...
	private CLIParser() {
		...
		globalPercent = Option.builder("p").desc("percentage of global messages (defaults to 0)").argName("percentage")
				.hasArg().numberOfArgs(1).type(Integer.class).build();
		duration = Option.builder("d").desc("time to execute in seconds (defaults to 120)").argName("seconds").hasArg()
				.numberOfArgs(1).type(Integer.class).build();
		msgSize = Option.builder("s").desc("message size in bytes (defaults to 64)").argName("bytes").hasArg()
				.numberOfArgs(1).type(Integer.class).build();
		clientCount = Option.builder("c").desc("number of client threads (defaults to 1)").argName("clients").hasArg()
				.numberOfArgs(1).type(Integer.class).build();
		treeConfig = Option.builder("t").desc("tree conf file").argName("file").hasArg().numberOfArgs(1)
				.type(String.class).required().build();
		nonGenuine = Option.builder("ng").desc("sets the server to the non-genuine config.uration").hasArg(false)
				.build();
		outstanding = Option.builder("o")
				.desc("number of outstanding messages allowed per client/replica (defaults to 1)").hasArg()
				.numberOfArgs(1).type(Integer.class).build();
		options = new Options();
