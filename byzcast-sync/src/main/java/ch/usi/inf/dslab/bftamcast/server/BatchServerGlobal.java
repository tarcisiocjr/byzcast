 ...
 * @author Tarcisio Ceolin - ceolin@ufsm.br
 */
public class BatchServerGlobal extends DefaultRecoverable {
    ...
    private BatchServerGlobal(int serverGlobalId, int groupId, String[] configPaths, String[] localConfigPaths) {
        ...
        System.out.println("******GENERIC GLOBAL GROUPS******");
        if (groupId == 0) {
            System.out.println("-.-.-.-TOP LEVEL-.-.-.-");
            proxies = new ServiceProxy[localConfigPaths.length];
            sleepTime = this.id * 1500;
...
//            ((GlobalBatchReplier) rep).setBatchSize(1);
            return command;
        }
    }
        ...
        try {
            ...
            for (int dest : allDest) {
                invokeThreads[dest] = new Thread(
                        () -> invokeReplies[dest] = proxies[dest].invokeOrdered(mainReq.toBytes()));
            ...
            for (int dest : allDest) {
                ...
                // System.out.println("reply from " + dest + ": req = " + auxReq + ", reply size
                // = " + temp.length);
