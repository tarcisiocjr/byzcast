...
for (( j = 1; j <=$(( G+1 )); j++ )); do
    for (( i = 1; i <= $N; i++ )); do
        ...
        if (( j == G+1 )); then
            ...
            tmux send-keys -t bftamcast.$PANE "$JAVA.server.ServerGlobal -i $(( i-1 )) -gc ./config/localhost/global0 $LCS $ARGS" C-m
        else
            tmux send-keys -t bftamcast.$PANE "$JAVA.server.Server -i $(( i-1 )) -g $(( j-1 )) -lc ./config/localhost/local$(( j-1 )) $ARGS" C-m
        fi
    done
done
