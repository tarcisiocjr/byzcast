...
while IFS=$'\t' read hostname client_id local_group_id num_of_clients num_of_groups; do
	...
	if [ "$hostname" = "$HOSTNAME" ]; then
        ...
        java -cp '../lib/*:bin/*' ch.usi.dslab.ceolin.bftswarm.ClientThroughput $client_id $local_group_id $num_of_clients $num_of_groups "config-local" "config-global" false 120000
	fi
